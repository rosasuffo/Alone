...
public class Phone1 : Phone
{
    public override void OnNotify(PlayerActions action)
    {
        switch (action)
        {
            case (PlayerActions.Phone1Reached):
                calling = true;
                whatCall = call.one;
                return;

            case (PlayerActions.Phone1Interaction):
    ...
    private void OnTriggerStay(Collider other)
    {
        if (ReferenceEquals(other, GetSubject().GetComponent<Collider>()))
        {
            ...
            if (Input.GetKeyDown(KeyCode.E)) GetSubject().NotifyObservers(PlayerActions.Phone1Interaction);
        }
    }
